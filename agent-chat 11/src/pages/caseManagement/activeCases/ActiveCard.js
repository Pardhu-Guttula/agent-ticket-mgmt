...
function ActiveCard({ searchQuery }) {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/api/cases?caseType=Accepted&agentId=${agentId}`
      );
                ...
                >
                  <div>
                    ...
                    <h2 className="break-words text-justify">
                      <span>{card.caseDescription}</span>
                    </h2>           
                  </div>
                </div>
              ))}
                  ...
                  {filteredCards.map((card) => (
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        ...
                        <span className="text-justify">{card.caseDescription}</span>
                      </div>
                    </div>
                  ))}
                </div>
