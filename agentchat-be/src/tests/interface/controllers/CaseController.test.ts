...
import { getCases, updateCaseStatus, createTicket, getTicketId, createCase } from '../../../interface/controllers/CaseController';
...
describe('Case Controller', () => {
  ...
  describe('PUT /api/cases/accept/:uniqueCaseId', () => {
    it('should update the status of a case to Accepted', async () => {
      ...
      jest.spyOn(CaseDbRepository.prototype, 'updateCaseStatusById').mockImplementation(async () => {
        return Promise.resolve(); 
      });
  
      const response = await request(app).put(`/api/cases/accept/${uniqueCaseId}`).send(updateData);
    ...
    it('should return 400 for missing agentId or caseType', async () => {
      ...
      const response = await request(app).put(`/api/cases/accept/${uniqueCaseId}`).send(invalidData);
    ...
    it('should return 500 for unexpected server errors', async () => {
      jest.spyOn(CaseDbRepository.prototype, 'updateCaseStatusById').mockImplementation(() => {
        throw new Error('Internal Server Error');
      });
  ...
  describe('POST /api/cases/create-ticket', () => {
    ...
    it('should return 500 for unexpected server errors', async () => {
      jest.spyOn(CaseDbRepository.prototype, 'createTicket').mockImplementation(() => {
        throw new Error('Internal Server Error');
      });
  ...
  describe('GET /api/cases/ticketId/:uniqueCaseId', () => {
    ...
    it('should return 404 if the case does not exist', async () => {
      const response = await request(app).get('/api/cases/ticketId/NonExistentCaseId');
      expect(response.status).toBe(404);
      expect(response.body.error).toBe('Ticket not found');
    });

    it('should return 500 for unexpected server errors', async () => {
      jest.spyOn(CaseDbRepository.prototype, 'getTicketByCaseId').mockImplementation(() => {
        throw new Error('Internal Server Error');
      });
  ...
  describe('POST /api/cases/createCase/:uniqueUserId', () => {
    ...
    it('should return 500 for unexpected server errors', async () => {
      jest.spyOn(CreateCaseCommand.prototype, 'execute').mockImplementation(() => {
        throw new Error('Internal Server Error');
      });
