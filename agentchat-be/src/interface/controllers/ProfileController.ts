...
export const getProfile = async (req: Request, res: Response) => {
  ...
  if (!agentId) {
    ...
    return res.status(400).json({ error: ApplicationError.ERROR_INVALID_AGENT_ID });
  }
  ...
  try {
    ...
    if (error instanceof ApplicationError && error.message === ApplicationError.ERROR_AGENT_NOT_FOUND) {
      
      return res.status(404).json({ error: ApplicationError.ERROR_AGENT_NOT_FOUND });
    }
